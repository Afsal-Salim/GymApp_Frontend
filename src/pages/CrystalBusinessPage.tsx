...
import GymClientBookTrialModal from './crystal/GymClientBookTrialModal';
import GymClientPlanVisitModal from './crystal/GymClientPlanVisitModal';
import {
...
import { buildGymClientWhatsAppHref } from './crystal/gymClientWhatsApp';
import { recordGymClientLeadCta, recordGymClientWhatsAppClick } from './crystal/gymClientLeadTracking';
import GymClientJoinLeadModal from './crystal/GymClientJoinLeadModal';
import { GymClientMidCtaIcon } from './crystal/GymClientDecorIcons';
      ...
      <Container>
        ...
        <Navbar.Collapse id="gym-client-nav">
          <Nav className="ms-auto align-items-lg-center gap-lg-2 py-2 py-lg-0">
            {content.nav.items
              .filter((item) => showTrainers || item.id !== 'trainers')
              .map((item) => (
...
function GymClientFooter({ content, showTrainers }: { content: GymClientSiteContent; showTrainers: boolean }) {
  return (
            ...
            <nav aria-label="Footer">
              <ul className="crystal-client-footer__links list-unstyled d-flex flex-wrap justify-content-center gap-3 mb-0 small">
                {content.footer.links
                  .filter((l) => showTrainers || l.href !== '#trainers')
                  .map((l) => (
                    <li key={l.label}>
...
function GymClientSiteView({ content, businessSlug }: { content: GymClientSiteContent; businessSlug: string }) {
  ...
  const joinLeadFromContactCta = content.nav.ctaHref === '#contact';
  ...
  const heroRating = content.header.ratingLine?.trim();
  ...
  return (
    <div ref={shellRef} className="crystal-client-shell" id="top">
        ...
        <Container>
          <div className="text-center mb-4" data-reveal>
            ...
            {content.discountOffers.sectionSubtitle ? (
              <p className="crystal-client__section-lead crystal-client__section-lead--on-dark mb-0">{content.discountOffers.sectionSubtitle}</p>
            ) : null}
          </div>
          <Row className="g-4 justify-content-center">
            {content.discountOffers.offers.map((offer, idx) => (
      ...
      {embedSrc ? (
        <section className="crystal-client__section crystal-client__section--video" id="tour" aria-labelledby="crystal-client-video">
          <Container>
            ...
            <div className="crystal-client__video-frame" data-reveal>
              <iframe
      ...
      {(visibleDetails.length > 0 || visibleContacts.length > 0) && (
        <section className="crystal-client__section crystal-client__section--meta" aria-labelledby="crystal-client-meta">
          <Container className="crystal-client__meta-container">
            <Row className="crystal-client__meta-row g-4 g-lg-5 justify-content-center">
              {visibleDetails.length > 0 ? (
                <Col xs={12} md={6} lg={5} id="visit" className="crystal-client__meta-col">
                  ...
                  <dl className="crystal-client__dl" data-reveal>
              ...
              {visibleContacts.length > 0 ? (
                <Col xs={12} md={visibleDetails.length ? 6 : 10} lg={visibleDetails.length ? 5 : 6} id="contact" className="crystal-client__meta-col">
                  <h2
                  ...
                  <ul className="crystal-client__contact-list list-unstyled mb-0" data-reveal>
                    {visibleContacts.map((item) => (
                      <li key={item.id} className="crystal-client__contact-item mb-4">
                        ...
                        {item.id === 'phone' && whatsappHref ? (
                          ...
                          <a href={item.href} className="crystal-client__link">
                            {item.value}
                          </a>
                        ) : (
                          <span className="crystal-client__contact-value">{item.value}</span>
                        )}
                      </li>
                    ))}
                  </ul>
      ...
      <GymClientFooter content={content} showTrainers={trainersList.length > 0} />
...
export default function CrystalBusinessPage() {
  ...
  useEffect(() => {
    ...
    const onStorage = (e: StorageEvent) => {
      if (e.key !== CRYSTAL_WEBSITE_PREVIEW_STORAGE_KEY && e.key != null) return;
