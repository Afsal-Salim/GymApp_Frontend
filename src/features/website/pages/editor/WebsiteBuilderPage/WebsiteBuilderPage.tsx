...
import { expandBlockManagerCategories, registerWebsiteBuilderExtensions } from '@/features/website/editor/blocks/websiteBuilderBlocks/websiteBuilderBlocks';
import { registerWebsiteBuilderLayerGroup } from '@/features/website/editor/blocks/websiteBuilderLayerGroup/websiteBuilderLayerGroup';
import { readCrystalWebsitePreviewFromStorage } from '@/features/website/setup/createWebsiteFormState/createWebsiteFormState';
import {
...
} from '@/features/website/editor/shared/websiteBuilderConstants/websiteBuilderConstants';
import { WEBSITE_BUILDER_DESIGN_SYSTEM_FONTS_IMPORT } from '@/features/website/editor/blocks/websiteBuilderDesignSystemFonts/websiteBuilderDesignSystemFonts';
import { WEBSITE_BUILDER_DESIGN_SYSTEMS_CSS } from '@/features/website/editor/blocks/websiteBuilderDesignSystems.css/websiteBuilderDesignSystems.css';
import {
...
} from '@/features/website/editor/shared/websiteBuilderPersistence/websiteBuilderPersistence';
import { attachCanvasInjectedComponentLibraryCss } from '@/features/website/editor/canvas/websiteBuilderCanvasInjectedLibraryCss/websiteBuilderCanvasInjectedLibraryCss';
import {
...
import { attachCanvasMarqueeSelect } from '@/features/website/editor/canvas/websiteBuilderCanvasMarqueeSelect/websiteBuilderCanvasMarqueeSelect';
...
const CANVAS_PCT_MIN = 20;
const CANVAS_PCT_MAX = 100;

function clampCanvasPct(v: number): number {
  const n = Math.round(v * 10) / 10;
  return Math.min(CANVAS_PCT_MAX, Math.max(CANVAS_PCT_MIN, n));
}

function formatCanvasPct(v: number): string {
  const n = Math.round(v * 10) / 10;
...
function readCanvasDimsLs(): { w: number; h: number } | null {
  ...
  try {
    ...
    // Stored as percentages so canvas scales with editor layout changes.
    if (w < CANVAS_PCT_MIN || h < CANVAS_PCT_MIN || w > CANVAS_PCT_MAX || h > CANVAS_PCT_MAX) return null;
    return { w: clampCanvasPct(w), h: clampCanvasPct(h) };
  } catch {
    return null;
  }
}
...
export default function WebsiteBuilderPage() {
  ...
  useEffect(() => {
    if (canvasDims) {
      setCanvasDimsLabel(`${formatCanvasPct(canvasDims.w)} × ${formatCanvasPct(canvasDims.h)}`);
    }
  }, [canvasDims]);
  ...
  useEffect(() => {
    const onMove = (e: MouseEvent) => {
      ...
      const maxW = slotRect ? Math.max(CANVAS_MIN_W, Math.floor(slotRect.width)) : 2400;
      const maxH = slotRect ? Math.max(CANVAS_MIN_H, Math.floor(slotRect.height)) : Math.max(CANVAS_MIN_H, Math.floor(window.innerHeight - 72));
      let nw = d.startW;
      let nh = d.startH;
      ...
      const wp = clampCanvasPct((nw / maxW) * 100);
      const hp = clampCanvasPct((nh / maxH) * 100);
      setCanvasDims({ w: wp, h: hp });
      setCanvasDimsLabel(`${formatCanvasPct(wp)} × ${formatCanvasPct(hp)}`);
    };
    const onUp = () => {
...
<script src="${leadScriptSrc}"></script>
  ...
  const startCanvasResize = useCallback(
    (e: React.MouseEvent, kind: 'se' | 'e' | 's') => {
      ...
      const slotW = slotRect ? Math.max(CANVAS_MIN_W, Math.round(slotRect.width)) : Math.round(r.width);
      const slotH = slotRect ? Math.max(CANVAS_MIN_H, Math.round(slotRect.height)) : Math.round(r.height);
      const baseW = Math.round(r.width);
      const baseH = Math.round(r.height);
      if (!canvasDims) {
        setCanvasDims({
          w: clampCanvasPct((baseW / slotW) * 100),
          h: clampCanvasPct((baseH / slotH) * 100),
        });
      }
  ...
  const resetCanvasSize = useCallback(() => {
    ...
    queueMicrotask(() => {
      ...
      if (el) {
        ...
        const slot = el.parentElement;
        const sr = slot?.getBoundingClientRect();
        const wp = sr?.width ? (b.width / sr.width) * 100 : 100;
        const hp = sr?.height ? (b.height / sr.height) * 100 : 100;
        setCanvasDimsLabel(`${formatCanvasPct(wp)} × ${formatCanvasPct(hp)}`);
      }
      const ed = editorRef.current;
      if (ed) runCanvasAutoFit(ed);
