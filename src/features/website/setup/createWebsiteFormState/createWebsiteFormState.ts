...
} from '../../../crystal/gymClientSiteContent/gymClientSiteContent';
import { getPresetArtworkUrlForColors } from '@/features/website/templates/websiteThemePresets/websiteThemePresets';
...
export function readCrystalWebsitePreviewFromStorage(): CrystalWebsiteDraftPayload | null {
  ...
  try {
    ...
    raw = window.localStorage.getItem(CRYSTAL_WEBSITE_PREVIEW_STORAGE_KEY);
    if (!raw) {
      raw = window.sessionStorage.getItem(CRYSTAL_WEBSITE_PREVIEW_STORAGE_KEY);
    }
    if (!raw) return null;
    return parsePreviewPayload(raw);
  } catch {
...
export function writeCrystalWebsitePreviewToStorage(payload: CrystalWebsiteDraftPayload): boolean {
  ...
  try {
    window.localStorage.setItem(CRYSTAL_WEBSITE_PREVIEW_STORAGE_KEY, JSON.stringify(payload));
    try {
      window.sessionStorage.removeItem(CRYSTAL_WEBSITE_PREVIEW_STORAGE_KEY);
    } catch {
      /* ignore */
    }
