...
function parseReturnPath(fromParam: string | null): LoginReturnTarget {
  if (!fromParam) return { pathname: '/user' };
  let decoded: string;
  try {
  ...
  if (q === -1) return { pathname: decoded };
  return { pathname: decoded.slice(0, q), search: decoded.slice(q) };
}

function buildReturnUrl(target: LoginReturnTarget): string {
  if (target.search) {
    const s = target.search.startsWith('?') ? target.search : `?${target.search}`;
    return `${target.pathname}${s}`;
  }
...
export default function LoginPage() {
  ...
  const state: LocationState = {
    ...
    from: parseReturnPath(searchParams.get('from')),
  };
            ...
            <div className="auth-page__form-inner">
              ...
              <Form onSubmit={handleSubmit}>
                ...
                <div className="auth-page__options d-flex justify-content-end mb-4">
                  <Link href="/forgot-password" className="auth-page__link small">Forgot password?</Link>
                </div>
                <Button
                  type="submit"
                  variant="primary"
                ...
                <p className="auth-page__terms small text-muted mb-0">
                  ...
                  <Link href="/" className="auth-page__link">Terms of Use</Link>.
                </p>
              </Form>
              <p className="auth-page__switch text-center mt-3 mb-0">
                Don&apos;t have an account?{' '}
                <Link href="/signup" className="auth-page__link">Sign up</Link>
              </p>
            </div>
          </Col>
